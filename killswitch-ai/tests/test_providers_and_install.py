...
class TestTopLevelImports:
    def test_guarded_openai_importable_from_killswitch_openai(self):
        from killswitch.openai import GuardedOpenAI  # noqa: F401
        assert GuardedOpenAI is not None

    def test_guarded_anthropic_importable_from_killswitch_anthropic(self):
        from killswitch.anthropic import GuardedAnthropic  # noqa: F401
        assert GuardedAnthropic is not None

    def test_guarded_openai_is_same_class_as_provider(self):
        from killswitch.openai import GuardedOpenAI as A
        from killswitch.providers.openai import GuardedOpenAI as B
        assert A is B

    def test_guarded_anthropic_is_same_class_as_provider(self):
        from killswitch.anthropic import GuardedAnthropic as A
        from killswitch.providers.anthropic import GuardedAnthropic as B
        assert A is B
...
class TestGuardedOpenAIWrapper:
    ...
    def test_allow_payload_calls_real_responses_create(self):
        from killswitch.providers.openai import GuardedOpenAI
        client = self._make_client()
        guard = GuardedOpenAI(client)
        cfg = Config(mode="report_only")

        with patch("killswitch.providers.openai.get_config", return_value=cfg), \
             patch("killswitch.providers.openai.scan_units") as mock_scan, \
             patch("killswitch.providers.openai.get_logger") as mock_logger, \
             patch("killswitch.providers.openai.reserve_event_id", return_value="EID-1"):
    ...
    def test_kill_action_raises_killswitch_blocked(self):
        ...
        with patch("killswitch.providers.openai.get_config", return_value=cfg), \
             patch("killswitch.providers.openai.scan_units") as mock_scan, \
             patch("killswitch.providers.openai.get_logger") as mock_logger, \
             patch("killswitch.providers.openai.reserve_event_id", return_value="EID-2"):
    ...
    def test_kill_action_logs_exactly_once_with_blocked_decision(self):
        ...
        with patch("killswitch.providers.openai.get_config", return_value=cfg), \
             patch("killswitch.providers.openai.scan_units") as mock_scan, \
             patch("killswitch.providers.openai.get_logger", return_value=mock_logger_instance), \
             patch("killswitch.providers.openai.reserve_event_id", return_value="EID-3"):
            mock_scan.return_value = MagicMock(findings=[finding], has_findings=True)
    ...
    def test_allow_action_logs_exactly_once_with_allow_decision(self):
        ...
        with patch("killswitch.providers.openai.get_config", return_value=cfg), \
             patch("killswitch.providers.openai.scan_units") as mock_scan, \
             patch("killswitch.providers.openai.get_logger", return_value=mock_logger_instance), \
             patch("killswitch.providers.openai.reserve_event_id", return_value="EID-4"):
            mock_scan.return_value = MagicMock(findings=[], has_findings=False)
    ...
    def test_drop_action_returns_synthetic_response_without_calling_provider(self):
        from killswitch.providers.openai import GuardedOpenAI
        client = self._make_client()
        guard = GuardedOpenAI(client)
        cfg = Config(mode="pause")
        cfg.actions["openai_key"] = "drop"

        finding = _make_finding()
        with patch("killswitch.providers.openai.get_config", return_value=cfg), \
             patch("killswitch.providers.openai.scan_units") as mock_scan, \
             patch("killswitch.providers.openai.get_logger") as mock_logger, \
             patch("killswitch.providers.openai.reserve_event_id", return_value="EID-DROP"):
            mock_scan.return_value = MagicMock(findings=[finding], has_findings=True)
            mock_logger.return_value = MagicMock()

            response = guard.responses.create(model="gpt-4o", input="sk-proj-abc123xyz789long")

        client.responses.create.assert_not_called()
        assert response._killswitch_dropped is True
...
class TestGuardedAnthropicWrapper:
    ...
    def test_kill_action_logs_once_with_blocked_decision(self):
        from killswitch.providers.anthropic import GuardedAnthropic
        client = self._make_client()
        ...
        finding = _make_finding()
        mock_logger_instance = MagicMock()
        with patch("killswitch.providers.anthropic.get_config", return_value=cfg), \
             patch("killswitch.providers.anthropic.scan_units") as mock_scan, \
             patch("killswitch.providers.anthropic.get_logger", return_value=mock_logger_instance), \
             patch("killswitch.providers.anthropic.reserve_event_id", return_value="EID-5"):
            mock_scan.return_value = MagicMock(findings=[finding], has_findings=True)
...
class TestRedactAllFindingTypes:
    ...
    def test_redact_covers_prohibited_term_finding(self):
        from killswitch.core.redactor import redact_text
...
class TestWizardCategoryPersistence:
    def test_high_entropy_off_disables_entropy_in_config(self):
        from killswitch.cli.wizard import _CATEGORY_TO_FINDING_TYPES
    ...
    def test_selected_category_keeps_default_action(self):
        from killswitch.cli.wizard import _CATEGORY_TO_FINDING_TYPES
        from killswitch.core.config import DEFAULT_ACTIONS
    ...
    def test_category_mapping_covers_all_data_categories(self):
        from killswitch.cli.wizard import DATA_CATEGORIES, _CATEGORY_TO_FINDING_TYPES
        wizard_keys = {category[0] for category in DATA_CATEGORIES}
